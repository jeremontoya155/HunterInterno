...
app.get('/vendedores', isAuthenticated, async (req, res) => {
        ...
        if (vendedorIds.length > 0) {
            ...
            desempenoMesResult.rows.forEach(item => {
                desempenoMesMap[item.vendedor_id] = {
                    ...
                    manuales: parseInt(item.total_manuales_mes, 10) || 0
                };
            });
        }
            ...
            desempenoHoyMap = desempenoHoyResult.rows.reduce((map, item) => {
                ...
                if(usernameLower) {
                    map[item.vendedor_id][usernameLower] = {
                        mensajes: item.mensajes_enviados || 0,
                        respuestas: item.respuestas_recibidas || 0,
                        mensajesManuales: item.mensajes_manuales || 0
                    };
                }
                return map;
            }, {});
    ...
    app.post('/vendedores/desempeno', isAuthenticated, async (req, res) => {
        ...
        try {
            ...
            // Bloqueo preventivo para evitar duplicados durante la transacción
            await client.query('SELECT pg_advisory_xact_lock($1)', [vendedor_id]);
            ...
            for (const item of desempeno) {
                ...
                await client.query(query, [
                    ...
                    fecha,
                    cuenta,
                    parseInt(item.mensajes, 10) || 0,
                    parseInt(item.respuestas, 10) || 0,
                    parseInt(item.mensajes_manuales, 10) || 0
                ]);
            }
            ...
            res.json({ 
                success: true, 
                message: 'Registros actualizados correctamente (sobrescritura completa).' 
            });
            ...
            res.status(500).json({ 
                success: false, 
                message: `Error al actualizar desempeño: ${error.message}` 
            });
        } finally {
            client.release();
        }
